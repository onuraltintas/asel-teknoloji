...
export class ServiceStatusComponent implements OnInit, OnDestroy {
  ...
  submitCreate(form: any) {
    if (
      ...
      !this.createForm.customerPhone ||
      !this.createForm.deviceType ||
      !this.createForm.issueDescription
    )
      return;
    ...
    const doSubmit = (token?: string) => {
      this.api.createTechnicalService({ ...this.createForm, recaptchaToken: token }).subscribe({
        next: (r) => {
          this.createdCode.set(r.serviceCode);
          this.creating.set(false);
        },
        error: () => {
          this.createError.set('Kayıt oluşturulurken bir hata oluştu. Lütfen tekrar deneyin.');
          this.creating.set(false);
        },
      });
    };
  ...
  search() {
    ...
    this.api.queryServiceStatus(trimmed).subscribe({
      ...
      error: () => {
        this.errorMsg.set(`"${trimmed}" koduna ait servis kaydı bulunamadı.`);
        this.searching.set(false);
      },
    });
  }
  ...
  isActive(s: number, status: number) {
    return status === s && s !== 4;
  }
  isCompleted(s: number, status: number) {
    return status !== 4 && s < status && s !== 4;
  }
  isPending(s: number, status: number) {
    return !this.isActive(s, status) && !this.isCompleted(s, status);
  }
