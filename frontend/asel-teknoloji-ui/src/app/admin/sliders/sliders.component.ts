      ...
      <div class="card overflow-x-auto">
        <table class="w-full text-sm">
          ...
          <tbody>
                ...
                <td class="p-2">
                  @if (item.imageUrl) {
                    <img [src]="item.imageUrl" [alt]="item.title"
                         class="w-16 h-10 object-cover rounded border border-gray-100"
                         onerror="this.style.display='none'" />
                  } @else {
                    <div class="w-16 h-10 bg-gray-100 rounded flex items-center justify-center text-gray-300 text-lg">🖼</div>
                  }
                </td>
            ...
            @empty {
              <tr><td colspan="7" class="p-6 text-center text-gray-400">Henüz slider yok.</td></tr>
            }
          </tbody>
        </table>
      </div>
...
export class SlidersComponent implements OnInit {
  ...
  load() {
    this.api.getSlidersAdmin().subscribe(d => {
      this.items = d;
      this.selected.clear();
  ...
  deleteSelected() {
    ...
    this.toast.confirm(`${ids.length} slider silinsin mi?`, () => {
      forkJoin(ids.map(id => this.api.deleteSlider(id))).subscribe({
        next: () => { this.load(); this.toast.success(`${ids.length} slider silindi.`); },
