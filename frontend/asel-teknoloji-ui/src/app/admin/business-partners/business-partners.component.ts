...
export class BusinessPartnersComponent implements OnInit {
  ...
  showForm = false;
  editing: BusinessPartner | null = null;
  ...
  load() {
    this.api.getBusinessPartnersAdmin().subscribe(d => {
      this.items = d;
      this.selected.clear();
  ...
  deleteSelected() {
    ...
    this.toast.confirm(`${ids.length} iş ortağı silinsin mi?`, () => {
      forkJoin(ids.map(id => this.api.deleteBusinessPartner(id))).subscribe({
        next: () => { this.load(); this.toast.success(`${ids.length} iş ortağı silindi.`); },
        error: () => { this.load(); this.toast.error('Bazı kayıtlar silinemedi.'); }
      });
    }, 'Sil');
  }

  openForm(item?: BusinessPartner) {
    this.editing = item ?? null;
    this.showForm = true;
    this.form.patchValue(item ?? { name: '', description: '', logoUrl: '', website: '', partnerType: '', displayOrder: 1, isActive: true });
  }
  ...
  onFileSelect(event: Event) {
    ...
    this.api.uploadImage(file, 'partner').subscribe({
      next: res => { this.form.patchValue({ logoUrl: res.url }); this.uploading.set(false); input.value = ''; },
  ...
  save() {
    ...
    const obs = this.editing
      ? this.api.updateBusinessPartner(this.editing.id, { ...dto, id: this.editing.id })
      : this.api.createBusinessPartner(dto);
    obs.subscribe({
      next: () => { this.showForm = false; this.load(); this.toast.success(this.editing ? 'İş ortağı güncellendi.' : 'İş ortağı oluşturuldu.'); },
      error: () => this.toast.error('Kayıt sırasında hata oluştu.')
    });
  }

  delete(item: BusinessPartner) {
    this.toast.confirm(`"${item.name}" silinsin mi?`, () => {
      this.api.deleteBusinessPartner(item.id).subscribe({
        next: () => { this.load(); this.toast.success('İş ortağı silindi.'); },
